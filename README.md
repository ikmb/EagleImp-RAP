...
*EagleImp-RAP* is implemented as a DNAnexus applet to run the [EagleImp](https://github.com/ikmb/eagleimp) software on the UK Biobank Research Analysis Platform (UKB-RAP) in order to enable haplotype phasing and genotype imputation using UK Biobank haplotype reference panels.
...
You can adjust the options to your need or add additional options such as `-iallowStrandFlip=true` to your needs. For more information on the available options see [User options](#user-options) below.
